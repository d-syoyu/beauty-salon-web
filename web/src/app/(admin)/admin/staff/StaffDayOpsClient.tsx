...
function AttendanceStatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    completed: { label: '退勤済', cls: 'bg-gray-100 text-gray-600 border-gray-200' },
  };
...
export default function StaffDayOpsClient({
  initialDate,
  initialDayOps,
  pendingRequestCount: initialPendingCount,
}: {
  initialDate: string;
  initialDayOps: { summary?: Record<string, number>; rows?: DayOpsRow[] };
  pendingRequestCount: number;
}) {
  const router = useRouter();
  const [, startTransition] = useTransition();
  const [selectedDate, setSelectedDate] = useState(initialDate);
  const [dayOps, setDayOps] = useState(initialDayOps);
  ...
  const handleDateChange = (date: string) => {
    ...
    // Update URL for RSC re-render (bookmarkable) and fetch new data
    startTransition(() => {
      router.push(`/admin/staff?date=${date}`);
    });
    void loadDate(date);
  };
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">今日の勤怠・シフト・申請</p>
        </div>
        <Button
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-emerald-600 text-white rounded-xl p-3.5 flex items-center gap-3">
            <div className="p-2 bg-white/20 rounded-lg flex-shrink-0"><Clock className="w-4 h-4" /></div>
            <div>
              <p className="text-sm font-semibold">今日の勤怠</p>
              <p className="text-xs opacity-75">打刻・出退勤</p>
            </div>
          </div>
          <Link href="/admin/staff/monthly" className="bg-white border border-gray-200 rounded-xl p-3.5 flex items-center gap-3 hover:bg-gray-50 transition-colors group">
            <div className="p-2 bg-indigo-50 rounded-lg flex-shrink-0 group-hover:bg-indigo-100 transition-colors"><CalendarDays className="w-4 h-4 text-indigo-600" /></div>
            <div>
              <p className="text-sm font-semibold text-gray-900">月次シフト</p>
              <p className="text-xs text-gray-400">シフト表作成</p>
            </div>
          </Link>
          <Link href="/admin/requests" className="relative bg-white border border-gray-200 rounded-xl p-3.5 flex items-center gap-3 hover:bg-gray-50 transition-colors group">
            <div className="p-2 bg-rose-50 rounded-lg flex-shrink-0 group-hover:bg-rose-100 transition-colors"><ClipboardList className="w-4 h-4 text-rose-600" /></div>
            <div>
          ...
          <Link href="/admin/staff/list" className="bg-white border border-gray-200 rounded-xl p-3.5 flex items-center gap-3 hover:bg-gray-50 transition-colors group">
            <div className="p-2 bg-gray-100 rounded-lg flex-shrink-0 group-hover:bg-gray-200 transition-colors"><Users className="w-4 h-4 text-gray-600" /></div>
            <div>
