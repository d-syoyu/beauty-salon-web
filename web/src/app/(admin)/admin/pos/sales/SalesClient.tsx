...
import { subscribeAdminShopChanged } from '@/lib/admin-shop-client';
...
export default function SalesClient({
  ...
  const [paymentMethods, setPaymentMethods] = useState<PaymentMethodOption[]>(initialData.paymentMethods);

  const fetchPaymentMethods = useCallback(async () => {
    try {
      const res = await fetch('/api/admin/payment-methods');
      const data = await res.json();
      const methods: PaymentMethodOption[] = (data.storePaymentMethods || [])
        .filter((m: { isActive: boolean }) => m.isActive)
        .map((m: { code: string; displayName: string }) => ({ code: m.code, displayName: m.displayName }));
  ...
  }, [endDate, paymentFilter, startDate]);

  useEffect(() => {
    return subscribeAdminShopChanged(() => {
      void fetchSales();
      void fetchPaymentMethods();
    });
  }, [fetchPaymentMethods, fetchSales]);
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <select value={paymentFilter} onChange={(e) => setPaymentFilter(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white">
              ...
              {paymentMethods.length > 0
                ? paymentMethods.map(m => <option key={m.code} value={m.code}>{m.displayName}</option>)
            ...
            <div key={sale.id} className="p-4 md:p-5 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">
                  ...
                  {' / '}{PAYMENT_METHOD_LABELS[sale.paymentMethod] || paymentMethods.find(m => m.code === sale.paymentMethod)?.displayName || sale.paymentMethod}
